...
class MultiGoalEnv(Env, Serializable):
    ...
    def __init__(self, goal_reward=10, actuation_cost_coeff=30,
        ...
        self.dynamic_plots = []
        self.global_goal_reached = [False] * len(self.goal_positions)
    ...
    def step(self, action):
        ...
        if done:
            for i, goal in enumerate(self.goal_positions):
                if np.linalg.norm(cur_position - goal) < self.goal_threshold:
                    self.global_goal_reached[i] = True
    ...
    def render(self, paths):
        ...
        self._env_lines = []
        positions = paths
        #positions = np.stack([info['pos'] for info in paths['infos']])
        #for path in paths:
    ...
    def log_diagnostics(self, paths):
        ...
        for path in paths:
            last_obs = path["observes"][-1]
            #last_obs = path["observations"][-1]
            for i, goal in enumerate(self.goal_positions):
                if np.linalg.norm(last_obs - goal) < self.goal_threshold:
                    self.global_goal_reached[i] = True
                    print('env:goals_reached', i)
                    
        #print('env:goals_reached', goal_reached.count(True))
    ...
    def print_goal_reached(self):
        return self.global_goal_reached.count(True)
